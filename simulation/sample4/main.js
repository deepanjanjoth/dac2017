...
function postImageUsingJQuery() {
    ...
    newobj.children().children().children().children().first().attr("src", dynamicImage);
...
function toggleCommentBlock(elemObj) {
    $(elemObj).parent().parent().next().toggle();
};


function saveComment(elemObj){
    var commentText = $(elemObj).parent().prev().children().first().val();

    var newElement = "<a href='#' class='list-group-item'>" + commentText + "</a>";
    $(elemObj).parent().prev().prev().children().first().append(newElement);

    $(elemObj).parent().prev().children().first().val("");
}
...
function ajaxDemojQuery(){
    var jqxhr = $.get("./data.json", {}, "json");
    jqxhr.done(function(res, status){
        console.log(res);
    });
